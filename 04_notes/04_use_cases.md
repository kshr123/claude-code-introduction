...
| 対象者 | 主なユースケース |
|--------|------------------|
| **新人エンジニア** | コードベースの理解、ベストプラクティスの学習 |
| **シニアエンジニア** | コードレビューの効率化、アーキテクチャ設計の支援 |
| **フロントエンド** | コンポーネント開発、バグ修正、レスポンシブ対応 |
| **バックエンド** | API開発、データベース最適化、MCP統合 |
| **DevOps/SRE** | CI/CD設定、インフラコード管理 |
| **QA/テスター** | テスト自動化、E2Eテスト作成 |

### 🔧 シチュエーション別ユースケース（5種類）

| シチュエーション | 概要 |
|-----------------|------|
| **レガシーコードのリファクタリング** | テストがない古いコードを安全に改善 |
| **緊急のバグ修正** | 本番環境の問題を迅速に解決 |
| **新技術の導入** | 新しいライブラリやフレームワークを評価・導入 |
| **マイクロサービスの開発** | モノリスから段階的に分離 |
| **書籍・リポジトリから学習** ⭐ | 参考コードを理解して自分で実装しながら学ぶ |

### 🏢 チーム・組織レベルのユースケース（2種類）

| ユースケース | 効果 |
|-------------|------|
| **コーディング規約の統一** | チーム全体でスタイルを統一、レビュー時間削減 |
| **ナレッジの共有** | カスタムコマンドで作業を標準化、属人化を解消 |

### 📊 その他

- **期待される効果（定量的）** - 開発速度、コード品質、学習コストの改善指標
- **成功事例** - スタートアップ、中規模企業での導入事例
- **社内導入シナリオ** - パイロット導入から全社展開までの3フェーズ
