...
### 🔑 重要：Claude Codeが登場する場所

このガイドでは**3つの技術**を扱います。Claude Codeが登場するのは一部だけです：

```
┌─────────────────────────────────────────────────────┐
│ 1️⃣ Git Worktree                                     │
│    = Gitの標準機能（Claude Code 不要）               │
│    複数ブランチを別ディレクトリで扱う                  │
└─────────────────────────────────────────────────────┘
              ↓ この中で
┌─────────────────────────────────────────────────────┐
│ 2️⃣ Claude Code CLI                                  │
│    = 🤖 Claude Codeが登場！                          │
│    Worktree内で対話的にコード生成・修正               │
└─────────────────────────────────────────────────────┘
              ↓ git push
┌─────────────────────────────────────────────────────┐
│ 3️⃣ GitHub Actions                                   │
│    = GitHubの標準機能（Claude Code 不要）            │
│                                                     │
│    ただし、追加で導入：                               │
│    ┌─────────────────────────────────────┐          │
│    │ Claude Code Action                 │          │
│    │ = 🤖 Claude Codeが登場！            │          │
│    │ Claudeが自動でPR/Issueを処理       │          │
│    └─────────────────────────────────────┘          │
└─────────────────────────────────────────────────────┘
```
...
| 技術 | Claude Code の有無 | Claude Code の役割 |
|------|-------------------|-------------------|
| **Git Worktree** | ❌ Gitの機能 | なし（Gitだけで動作） |
| **Worktree内での開発** | ✅ **Claudeが登場** | 対話的なコード生成・修正 |
| **GitHub Actions** | ❌ GitHubの機能 | なし（GitHubだけで動作） |
| **GitHub Actions + Claude Code Action** | ✅ **Claudeが登場** | 自動PR/Issueレビュー・修正 |

### 全体像（Claude Codeの登場箇所をハイライト）
...
│  │  (feature-a)  │  │  (feature-b)  │  │
│  │               │  │               │  │
│  │ 🤖 Claude Code│  │ 🤖 Claude Code│  │ ← Claude登場！
│  │   CLI実行     │  │   CLI実行     │  │
│  └───────────────┘  └───────────────┘  │
└─────────────────────────────────────────┘
              ↓ git push
...
| 機能 | Git Worktree | Claude Code CLI | GitHub Actions | Claude Code Action |
|------|-------------|-----------------|----------------|-------------------|
| **実行場所** | ローカル | ローカル | クラウド | クラウド |
| **Claude有無** | ❌ なし | ✅ **あり** | ❌ なし | ✅ **あり** |
| **目的** | 並列開発環境 | コード生成・修正 | CI/CD自動化 | PR/Issue自動処理 |
| **起動方法** | `git worktree add` | `claude` コマンド | PR/Issue作成時 | 自動トリガー |
...
> **重要**: Git Worktree は **Gitの標準機能** です。Claude Code は不要で、単体で使えます。

**定義**: 同じリポジトリの複数ブランチを別ディレクトリで同時に扱える機能

#### 通常のGitワークフロー（❌ Claude Codeなし）
...
# Vim/VSCodeなど通常のエディタで編集
vim src/feature-b.js
```

#### Git Worktreeの場合（❌ Claude Codeなし）
...
**この時点では Claude Code は登場していません！**

### 🎯 Claude Codeとの組み合わせ（✅ ここでClaudeが登場）

Git Worktree **内** で Claude Code CLI を使うと、並列開発が劇的に改善されます：

#### 問題: 🤖 Claude Codeでコンテキストの汚染

Claude Code CLI を使う場合の問題：

```bash
# feature-a 実装中
cd myapp
claude "認証機能を実装して"    # 🤖 Claude Code 起動
# → Claude が feature-a のコンテキストを保持

# 別タスクへ切り替え
git stash
git checkout feature-b
claude "レポート機能を実装して"  # 🤖 Claude Code 起動
# → Claude が feature-a のコンテキストを混同する可能性
# → ファイルが存在しないエラーが発生
```

**Claude Code が混乱する理由**:
- 同じディレクトリで異なるブランチを切り替え
- Claude が以前のファイル構造を記憶
- 存在しないファイルを参照しようとする

#### 解決策: Worktree + 🤖 Claude Codeで完全分離
...
# feature-b 専用 worktree（同時進行、別ターミナル）
cd ~/projects/myapp-feature-b
claude "レポート機能を実装して"  # 🤖 別のClaude Codeセッション起動
# → feature-b 専用のコンテキスト
...
**メリット**:
- ✅ 各Worktreeで独立した Claude Code セッション
- ✅ コンテキスト混同なし
- ✅ 3-5個のタスクを真に並行開発

### 🚀 基本操作（❌ Claude Code不要、Gitのみ）

#### Worktree作成

Git Worktree は Git コマンドのみで動作します。Claude Code は**まだ登場しません**：
...
# ❌ Claude Code不要 - これはGitコマンド

# 既存ブランチで worktree 作成
git worktree add ../myapp-hotfix hotfix-123
# ❌ Claude Code不要 - これもGitコマンド
...
# 2. 認証機能用 worktree
git worktree add ../myapp-auth -b feature/auth  # ❌ Git操作（Claude不要）
cd ../myapp-auth
npm install                                      # ❌ npm操作（Claude不要）
claude "OAuth 2.0 認証を実装して"                 # ✅ 🤖 Claude Code 登場！

# 3. レポート機能用 worktree（別ターミナル）
cd ~/projects/myapp
git worktree add ../myapp-reports -b feature/reports  # ❌ Git操作
cd ../myapp-reports
npm install                                            # ❌ npm操作
claude "月次レポート機能を実装して"                     # ✅ 🤖 Claude Code 登場！

# 4. ホットフィックス用 worktree（緊急対応）
cd ~/projects/myapp
git worktree add ../myapp-hotfix -b hotfix/login-bug   # ❌ Git操作
cd ../myapp-hotfix
npm install                                             # ❌ npm操作
claude "ログインエラーを修正して"                        # ✅ 🤖 Claude Code 登場！

# 全て同時進行！
# → 3つの独立したClaude Codeセッションが動作
```

**重要な理解**:
- `git worktree add`: Gitの機能（Claude不要）
- `npm install`: npmの機能（Claude不要）
- `claude "..."`: 🤖 **ここでClaude Codeが登場**
...
### ⚡ 自動化スクリプト（✅ Claude Code使用）

#### スクリプト1: worktree + 🤖 Claude Code 起動

このスクリプトは Git Worktree作成 + Claude Code起動を自動化します：
...
# 環境セットアップ（❌ npm/pip/go操作、Claude不要）
